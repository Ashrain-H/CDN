...
Modified by: Astell the Cocowolf
*/
...
const walls = [
  ...
  { column: 0, row: 9, horizontal: true, length: 9 },
  ...
  { column: 5, row: 3, horizontal: true, length: 2 },
  ...
  { column: 4, row: 1, horizontal: false, length: 4 },
...
function resetGame() {
  ...
  if (hardMode) {
    noteElement.innerHTML = `点击摇杆开始(不用拖拽)
        <p>困难模式</p><p>按E返回简单模式</p>`;
  } else {
    noteElement.innerHTML = `点击摇杆开始(不用拖拽)
        <p>简单模式</p><p>按H开启困难模式</p>`;
  }
...
function main(timestamp) {
  ...
  try {
    ...
    if (
      balls.every(
        (ball) => distance2D(ball, { x: 350, y: 315 }) < 65 / 2
      )
    ) {
      noteElement.innerHTML = `恭喜过关
        ${!hardMode && "<p>按H开启困难</p>"}
        <p>
          Follow me
          <a href="https://space.bilibili.com/65394319" , target="_blank"
            >@大以巴狼艾斯</a
          >
        </p>`;
      noteElement.style.opacity = 1;
    ...
    if (error.message == "The ball fell into a hole") {
      noteElement.innerHTML = `球 掉出了这个世界_(:Dゝ∠)_
        <p>
          按空格重开,按E返回简单
        </p>`;
      noteElement.style.opacity = 1;
